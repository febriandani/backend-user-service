...
func (d *DBHandler) ConnectDB(dbBs *DBDetailUser) {
	...
	if err != nil {
		log.Error(ConnectDBFail + " | " + err.Error())
		d.Err = err
	}
...
func (d *DBHandler) Close() {
	if err := d.DB.Close(); err != nil {
		d.log.Println(ClosingDBFailed + " | " + err.Error())
	} else {
		d.log.Println(ClosingDBSuccess)
	}
}
